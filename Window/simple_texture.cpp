...
//
//
TEX *LoadTexture(std::string const &filename, std::string const &archname, int numcomponents, int filter)
{
	...
	else
	{
		...
		image_data = stbi_load_from_memory(loadGenericZip(archname.c_str(), filename.c_str()), (int)getlastZsize, &width, &height, &comp, STBI_rgb_alpha);
	}
	if (!image_data) {
